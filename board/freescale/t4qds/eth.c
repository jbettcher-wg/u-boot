 ...
 *
 * T4240QDS Ethernet — QIXIS and RGMII focus
 *
 * QIXIS BRDCFG4[7:5] (BRDCFG4_EMISEL_*) selects which EMI1 MDIO segment is
 * routed to the FMan MEMAC MDIO master; see t4240qds_mux_mdio() and
 * t4240qds_qixis.h. Values match the mdio-mux in
 * dts/upstream/src/powerpc/fsl/t4240qds.dts (mdio-mux-emi1 @ reg 0x54).
 *
 * Onboard RGMII (not SerDes slot SGMII): FM1@DTSEC5 uses PHY MDIO address 2 on
 * the QIXIS EMI1 RGMII mux (Linux mdio-mux mdio@0). FM2@DTSEC5 uses PHY 1.
 * U-Boot's trimmed t4240qds.dts enables only FM1@DTSEC5; Linux brings up the rest.
 * RCW fields (e.g. rcwsr13 EC1/EC2) must select RGMII for those ports; U-Boot
 * discovers the interface mode via fman_port_enet_if() in drivers/net/fm/t4240.c.
 *
 * Bring-up variants that do not use SerDes1/2 still need a valid RCW SerDes
 * protocol word for software that keys off srds_prtcl_s1/s2; the loops below
 * also program RGMII based on the interface mode returned for each port.
 */
...
/* Provided by drivers/net/fm/<soc>.c */
extern phy_interface_t fman_port_enet_if(enum fm_port port);
...
{
	int interface = fman_port_enet_if(port);
