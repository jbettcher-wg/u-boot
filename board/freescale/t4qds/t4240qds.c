...
int board_mux_pca9547(u8 ch, int bus_num)
{
	...
	ret = i2c_get_chip_for_busnum(bus_num, I2C_MUX_PCA_ADDR_PRI, 0, &dev);
	if (ret) {
		printf("%s: Cannot find I2C bus %d\n", __func__,
		       bus_num);
		return ret;
	}
...
/*
 * Runs in EVT_MISC_INIT_F: after initf_dm (DM) and before dram_init.
 * Legacy: i2c_init_all() has not run yet — do not touch the mux here; only
 * clear IRE via QIXIS. DM: mux select is safe; print one "I2C: ready" here.
 * dram_init() clears IRE again and selects the mux before fsl_ddr_sdram().
 */
static int t4240qds_misc_init_f(void)
{
	/* See ddr.c dram_init(): IRE must be off before local PCA9547 / SPD. */
...
EVENT_SPY_SIMPLE(EVT_MISC_INIT_F, t4240qds_misc_init_f);
...
static int adjust_vdd(ulong vdd_override)
{
	...
	ret = board_mux_pca9547(I2C_MUX_CH_VOL_MONITOR, 0);
	if (ret) {
...
exit:
	/*
	 * adjust_vdd() selects I2C_MUX_CH_VOL_MONITOR at entry; System ID EEPROM
	 * (0x57), RCW, SPD, etc. live on CH0 — restore before mac_read_from_eeprom().
	 */
	(void)board_mux_pca9547(I2C_MUX_CH_DEFAULT, CONFIG_SYS_SPD_BUS_NUM);
	if (re_enable)
		enable_interrupts();
...
int config_frontside_crossbar_vsc3316(void)
{
	...
	ret = board_mux_pca9547(I2C_MUX_CH_VSC3316_FS, 0);
	if (ret)
		return ret;
...
int board_early_init_r(void)
{
	...
	/*
	 * Route EMI1 MDIO to onboard RGMII (BRDCFG4[7:5]=0). Required before DM
	 * probes fm1-mac5 / mdio@5fc000 — legacy mux buses are not used.
	 */
	{
		u8 brdcfg4 = QIXIS_READ(brdcfg[4]);

		brdcfg4 &= ~BRDCFG4_EMISEL_MASK;
		brdcfg4 |= (0 << BRDCFG4_EMISEL_SHIFT); /* EMI1_RGMII */
	...
	board_mux_pca9547(I2C_MUX_CH_DEFAULT, 0);
#endif
...
{
	u8 sw;
	void *srds_base = (void *)CFG_SYS_FSL_CORENET_SERDES_ADDR;
...
#if defined(CONFIG_ID_EEPROM) && CONFIG_IS_ENABLED(DM_ETH)
	/*
	 * NXID EEPROM stores MACs in port order; FM1@DTSEC5 is index FM1_DTSEC5.
	 * mac_read_from_eeprom() fills ethaddr from slot 0 (first port); minimal
	 * U-Boot uses only DTSEC5 — copy that MAC into ethaddr (env_set: always
	 * override; eth_env_set_enetaddr rejects an existing ethaddr).
	 */
	{
		unsigned char m[ARP_HLEN];
		char ea[ARP_HLEN_ASCII + 1];

		if (eth_env_get_enetaddr_by_index("eth", FM1_DTSEC5, m)) {
			sprintf(ea, "%pM", m);
			env_set("ethaddr", ea);
		}
	}
...
void board_detail(void)
{
	...
	/* Voltage secion */
	if (!board_mux_pca9547(I2C_MUX_CH_VOL_MONITOR, 0)) {
		vdd = read_voltage();
		if (vdd > 0)
